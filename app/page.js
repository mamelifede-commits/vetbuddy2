...
function OwnerDocuments({ documents }) {
  return <div><h2 className="text-2xl font-bold text-gray-800 mb-2">I miei documenti</h2><p className="text-gray-500 text-sm mb-6">Referti, prescrizioni e istruzioni dalla clinica</p>{documents.length === 0 ? <Card><CardContent className="p-12 text-center text-gray-500"><FileText className="h-12 w-12 mx-auto mb-4 text-gray-300" /><p className="font-medium">Nessun documento</p></CardContent></Card> : <div className="space-y-3">{documents.map((doc) => <Card key={doc.id}><CardContent className="p-4 flex items-center justify-between"><div className="flex items-center gap-4"><div className="h-12 w-12 bg-blue-100 rounded-lg flex items-center justify-center"><FileText className="h-6 w-6 text-blue-600" /></div><div><p className="font-medium">{doc.name}</p><p className="text-sm text-gray-500">{doc.petName} • {new Date(doc.createdAt).toLocaleDateString()}</p></div></div><Button variant="outline" size="sm"><Download className="h-4 w-4 mr-1" />Scarica</Button></CardContent></Card>)}</div>}</div>;
}
